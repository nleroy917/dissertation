...
A first class of methods reduces the quadratic cost of self-attention by approximating the attention matrix with low-rank or kernel-based decompositions. Linformer projects the sequence dimension into a lower-rank space, yielding linear complexity while retaining competitive accuracy on many NLP tasks @Wang2020a. Nyströmformer applies the Nyström method to approximate the softmax kernel, further reducing memory usage while scaling to longer contexts @Xiong2021. Performer takes a different approach, introducing FAVOR+ random feature maps to approximate the softmax kernel directly in linear time @Choromanski2021. These methods sacrifice exactness but offer strong trade-offs between efficiency and fidelity.

==== Sparse attention mechanisms
Another approach is to restrict each token’s receptive field to a subset of positions, exploiting the intuition that not all pairwise interactions are necessary. Sparse Transformer introduced block-sparse patterns to limit computations without major accuracy loss @Child2019. Longformer extended this idea with a sliding-window local attention pattern augmented by global tokens for tasks like question answering @Beltagy2020. BigBird combined local, global, and random sparse connections, providing both empirical scalability and theoretical guarantees of universality @Zaheer2021. These structured sparsity patterns allow transformers to process sequences with tens of thousands of tokens while retaining exact attention within the restricted subsets.

==== Algorithmic improvements
A final line of work focuses on re-engineering the exact attention operation itself to be more memory- and bandwidth-efficient. FlashAttention reformulates attention as a sequence of IO-aware matrix multiplications, using tiling and recomputation to eliminate the need to materialize the full attention matrix in GPU memory @Dao2022. FlashAttention v2 further generalized these kernels for both training and inference, showing substantial speedups on modern accelerators @Dao2023. Related libraries, such as xFormers, provide a general framework for fused attention kernels with minimal memory overhead. Unlike low-rank or sparse approaches, these methods compute attention exactly but achieve significant gains by exploiting hardware efficiency.
