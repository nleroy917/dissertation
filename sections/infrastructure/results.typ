...
To facilitate this process, our group developed and re-implemented and maintain `uniwig`, a high-performance tool in rust that computes the coverage of genomic intervals across multiple datasets and generates a unified representation. `uniwig` takes as input a set of BED files representing genomic intervals from different experiments and computes three single coverage `.bigWig` files, one for the starts, the cores, and the ends of the intervals. This representation captures the distribution of intervals across the genome and can be used to identify regions that are consistently covered across datasets. The output `.bigWig` files can then be used as input for downstream analysis or as part of the vocabulary creation process (@gtokenizers-uniwig\A, #link(<infrastructure-uniwig-methods>, "See methods")).
